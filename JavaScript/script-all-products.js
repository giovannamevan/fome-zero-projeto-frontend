...
console.log("to usando esse script")
let produtos = []
async function getOngs(endpoint) {
    try {
...
window.onload = async () => {
    ...
    const fetchEndpoint = baseApiUrl + "products/";
    const fetchOngsEndpoint = baseApiUrl + "ONG/"
    const ongs = await getOngs(fetchOngsEndpoint)
    let ongsProducts = []
    ongs.forEach(ong => {
        ong.interesses.forEach(product => {
            ongsProducts.push(product)
        })

    })
    produtos = await getProducts(fetchEndpoint)

    const notInInterestProducts = produtos.filter(valor => ongsProducts.indexOf(valor) === -1);


    notInInterestProducts.forEach(async produto => {
